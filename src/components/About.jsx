    ...
    <section id="about" className="py-20 px-4 bg-gradient-to-br from-white via-orange-50 to-white dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 transition-colors duration-300">
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-12">
        ...
        <div className="flex-1 flex flex-col items-center md:items-start">
          ...
          <h3 className="text-[var(--color-orange)] font-semibold uppercase text-sm mb-2">{t('nav.about')}</h3>
          ...
          <p className="text-gray-700 dark:text-gray-300 mb-2 text-center md:text-left">
            {language === 'fr' 
              ...
              : "I am Mouhamadou Moustapha Gueye, passionate about technological innovation serving humanity."}
          </p>
          <p className="text-gray-700 dark:text-gray-300 mb-4 text-center md:text-left">
            {language === 'fr' 
              ...
              : "Former medical student turned software engineer and specialized in AI/Data, I combine science, tech, and fieldwork to create high-impact solutions."}
          </p>
        </div>
        {/* Timeline & Stats */}
        <div className="flex-1 flex flex-col gap-8">
          ...
          <div className="relative pl-6 border-l-2 border-[var(--color-orange)] space-y-6">
            <div>
              ...
              <p className="font-semibold dark:text-white">{language === 'fr' ? "Études de Médecine (jusqu'en 3ème Année)" : "Medical Studies (up to 3rd Year)"}</p>
              <p className="text-sm text-gray-600 dark:text-gray-400">{language === 'fr' ? "Formation scientifique & humaine" : "Scientific & human training"}</p>
            </div>
            <div>
              ...
              <p className="font-semibold dark:text-white">{language === 'fr' ? "Ingénierie & IA" : "Engineering & AI"}</p>
              <p className="text-sm text-gray-600 dark:text-gray-400">{language === 'fr' ? "Spécialisation en IA/Data & développement logiciel" : "Specialization in AI/Data & Software Development"}</p>
            </div>
            <div>
              ...
              <p className="font-semibold dark:text-white">{language === 'fr' ? "Impact Afrique" : "Africa Impact"}</p>
              <p className="text-sm text-gray-600 dark:text-gray-400">{language === 'fr' ? "Solutions tech pour la santé, l’agriculture, l’éducation" : "Tech solutions for health, agriculture, and education"}</p>
            </div>
          </div>
          {/* Stats */}
          <div className="grid grid-cols-2 gap-6 mt-4">
            <div className="flex flex-col items-center bg-white dark:bg-gray-800 rounded-lg shadow p-4 transition-colors">
              ...
              <span className="text-xs text-gray-600 dark:text-gray-400 text-center">{language === 'fr' ? "Années d’expérience" : "Years of Experience"}</span>
            </div>
            <div className="flex flex-col items-center bg-white dark:bg-gray-800 rounded-lg shadow p-4 transition-colors">
              <FaLaptopCode className="text-2xl text-[var(--color-orange)] mb-1" />
              <span className="text-xl font-bold text-[var(--color-orange)]">20+</span>
              <span className="text-xs text-gray-600 dark:text-gray-400 text-center">{language === 'fr' ? "Projets réalisés" : "Completed Projects"}</span>
            </div>
            <div className="flex flex-col items-center bg-white dark:bg-gray-800 rounded-lg shadow p-4 transition-colors">
              <FaGlobeAfrica className="text-2xl text-[var(--color-orange)] mb-1" />
              <span className="text-xl font-bold text-[var(--color-orange)]">3</span>
              <span className="text-xs text-gray-600 dark:text-gray-400 text-center">{language === 'fr' ? "Pays d’intervention" : "Countries Impacted"}</span>
            </div>
            <div className="flex flex-col items-center bg-white dark:bg-gray-800 rounded-lg shadow p-4 transition-colors">
              <FaBrain className="text-2xl text-[var(--color-orange)] mb-1" />
              <span className="text-xl font-bold text-[var(--color-orange)]">15+</span>
              <span className="text-xs text-gray-600 dark:text-gray-400 text-center">{language === 'fr' ? "Technologies maîtrisées" : "Mastered Technologies"}</span>
            </div>
          </div>
        </div>
      </div>
